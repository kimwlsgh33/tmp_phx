  ...
  def init_upload(access_token) do
    ...
    case HTTPoison.post(url, "{}", headers) do
        ...
        case Jason.decode(body) do
          ...
          {:error, _} ->
            ...
            {:error, ErrorHandler.error(
              :api_error,
              "Failed to parse TikTok response",
              %{body: body},
              __MODULE__
            )}
        end
      ...
      {:ok, %HTTPoison.Response{status_code: status_code, body: body}} ->
        ...
        context = %{endpoint: "/video/upload/"}
        ApiError.handle_response(status_code, body, :tiktok, context)

      {:error, %HTTPoison.Error{} = error} ->
        # Use our standardized HTTP error handling
        ApiError.handle_http_error(error, :tiktok, %{endpoint: "/video/upload/"})
    end
  end
  ...
  def upload_chunk(upload_id, chunk_data, chunk_index, chunk_start, chunk_end, total_size, access_token) do
    ...
    case HTTPoison.post(url, chunk_data, headers) do
      ...
      {:error, %HTTPoison.Error{} = error} ->
        ...
        ApiError.handle_http_error(error, :tiktok, context)
    end
  end
  ...
  def finalize_upload(upload_id, title, privacy_level, disable_comments, disable_duet, disable_stitch, access_token) do
    ...
    case HTTPoison.post(url, body, headers) do
        ...
        case Jason.decode(response_body) do
          ...
          {:error, _} ->
            ...
            {:error, ErrorHandler.error(
              :api_error,
              "Failed to parse TikTok response",
              %{body: response_body, upload_id: upload_id},
              __MODULE__
            )}
        end
      ...
      {:ok, %HTTPoison.Response{status_code: status_code, body: body}} ->
        # Use our standardized API error handling
        context = %{
          endpoint: "/video/upload/complete/",
          upload_id: upload_id
        }
        ApiError.handle_response(status_code, body, :tiktok, context)
      ...
      {:error, %HTTPoison.Error{} = error} ->
        # Use our standardized HTTP error handling
        context = %{endpoint: "/video/upload/complete/", upload_id: upload_id}
        ApiError.handle_http_error(error, :tiktok, context)
    end
  end
    ...
    case validate_access_token(opts) do
      {:ok, access_token} ->
        ...
        case HTTPoison.get(url, headers) do
          ...
          {:ok, %HTTPoison.Response{status_code: status_code, body: body}} ->
            # Use our standardized API error handling
            context = %{endpoint: "/video/info/", video_id: video_id}
            ApiError.handle_response(status_code, body, :tiktok, context)
